...
## Deployment WEB-App
First of all you need to be connected to the EC2 server for `staging or production` environment. Then open `webapp` folder. After that you need to check all changes from the Git. If data is not already up to date, make `git pull` with your credentials. We use `pm2` util for running __Next.js__ app in the EC2. Execute next command for updating next.js app:
```sh
cd webapp
git pull # check your branch
yarn install
yarn build
pm2 kill
...
```
After that steps you will has updated webapp
...
1. Open `undercard_18898.xcodeproj`in xcode.
