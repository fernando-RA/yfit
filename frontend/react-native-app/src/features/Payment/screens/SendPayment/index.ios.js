...
stripe.setOptions({
  ...
  androidPayMode: 'production', // Android only
});
...
class App extends Component {
  ...
  createCard = async val => {
    ...
    fetch('https://api.stripe.com/v1/tokens', {
      ...
      .then(json => {
        if (json.error) {
          console.log('createCard::', json.error);
  ...
  handleApplePayPress = async () => {
    ...
    if (!this.state.amount) {
      setTimeout(() => {
        Alert.alert(
          appInfo.appName,
          'Please enter the amount to continue apple pay..',
        );
      }, 300);
      return;
    }
    ...
    if (!canMakePayment) {
      console.log('handleApplePayPress:: ', canMakePayment);
      Alert.alert(appInfo.appName, 'Can not make Apple payment, please setup', [
        {
          text: 'Ok',
    ...
    try {
      ...
      console.log('handleApplePayPress::: error', error);
  ...
  _addPaymentType(op) {
    if (op === 'Apple Pay') {
      console.log('_addPaymentType:', op);
